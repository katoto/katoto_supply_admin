<template>
  <view
    class="order has-fixed-bottom"
    :class="{ 'has-fixed-bottom-iphoneX': isIphoneX }"
  >
    <view
      v-if="!pending"
      class="flex justify-center bg-blue padding"
    >
      <view class="text-center">
        <text
          class="order-status-icon"
          :class="ORDER_STATUS_ICON[order.status]"
        ></text>
        <text
          class="order-status text-xl"
    ...
    <view class="cu-list menu">
      ...
      >
        <view
          v-if="order.addressUuid"
          class="content padding-tb-sm"
        >
          <view>
    ...
    <view class="cu-list menu-avatar">
      <view
        class="cu-item"
        v-for="(goods, index) in order.lines"
        :key="index"
      >
        <view
          class="cu-avatar lg"
          :style="goods.goodsPic ? 'background-image:url(' + goods.goodsPic + ');' : ''"
        ></view>
        <view class="content">
          <view class="text-cut">
            {{goods.goodsName}}
    ...
    <!--底部-->
    <view
      class="cu-bar bg-white shop fixed-bottom"
      :class="{ 'fixed-bottom-iphoneX': isIphoneX }"
    >
      <view
        v-if="pending"
        class="flex order-footer-pending"
      >
        <text class="text-red">￥
      ...
      <view
        v-else
        class="flex order-footer"
      >
        <button
          v-if="isInitial"
          @tap="cancelBill"
          class="cu-btn round text-grey line-gray margin-left-sm"
        >取消订单
        </button>
        <button
          v-if="isInitial"
          @tap="pay"
          class="cu-btn round text-red line-red margin-left-sm"
        >去支付
        </button>
        <button
          v-else
          @tap="orderAgain(order.lines)"
          class="cu-btn round text-black line-black margin-left-sm"
        >再次购买
        </button>
        <button
          v-if="order.status === 'dispatching'"
          @tap="completeBill"
          class="cu-btn round text-red line-red margin-left-sm"
        >确认收货
        </button>
