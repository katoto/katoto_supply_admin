...
<template>
  <view
    class="goods-box"
    :class="{ 'goods-scroll': scroll, relative: height === 'auto' }"
  ...
  >
    ...
    <scroll-view
      ...
      scroll-y
      scroll-with-animation
      :scroll-into-view="'goods-' + mainCur"
      @scroll="handleVerticalMainScroll"
    >
      <!--无数据-->
      <view
        v-if="emptyOptions && (getDatasLen(datas) === 0)"
        class="goods-empty"
      >
        <view class="text-center">
          <image
            class="goods-empty-icon"
            :src="emptyOptions.icon"
          ></image>
      ...
      <view
        v-else
        :class="{ 'padding-right padding-bottom padding-left': scroll }"
      >
        <view
          ...
          <view :class="{ 'shadow-normal': scroll }">
            ...
            >
              ...
              >
                ...
                <text
                  v-if="!selectable"
                  class="bg-blue"
                  :style="'width:' + (getStrRealLen(item.label) * 2) + 'em'"
                ></text>
              </view>
            </view>
            <view
              v-if="group && getDatasLen(item.lines) === 0"
              class="cu-list menu"
            >
              <view class="cu-item">
                <view class="content">
            ...
            >
              ...
              >
                <text
                  v-if="selectable"
                  class="goods-item-select"
                  :class="goods.checked ? 'cuIcon-roundcheckfill text-red' : 'cuIcon-round text-grey'"
                  @tap="selectGoods(key, index)"
                ></text>
                <view
                  class="cu-avatar lg goods-item-thumb"
                  :style="goods.thumbnail ? 'background-image:url(' + goods.thumbnail + ');' : ''"
                ></view>
                ...
                <view class="goods-item-right">
                  ...
                  <view
                    ...
                    class="cuIcon-cart text-blue right-icon"
                    @click="rightIconTap(goods, key)"
                  ></view>
                  <uni-number-box
                    v-else-if="right === 'number'"
                    :value="goods.quantity"
                    :min="1"
                    @change="numChange($event, key, index)"
                  ></uni-number-box>
                </view>
                <view
                  class="move"
                  @tap="remove(key, index)"
                >
    ...
    <!--底部-->
    <view
      v-if="selectable && Object.keys(datas).length > 0"
